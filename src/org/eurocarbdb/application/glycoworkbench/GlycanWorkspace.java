...
public class GlycanWorkspace extends BuilderWorkspace implements
		SAXUtils.SAXWriter {
	...
	public static class Event extends java.util.EventObject {
		...
		public Event(GlycanWorkspace _source, Scan _parent, Scan _child,
				int _index, Scan _current) {
			super(_source);
			parent = _parent;
			child = _child;
			index = _index;
	...
	public Collection<javax.swing.filechooser.FileFilter> getFileFormats() {
		...
		filters.add(new ExtensionFileFilter("gwp",
				"GlycoWorkbench workspace file"));
		return filters;
	}
	...
	public boolean syncScan(Scan scan) {
		...
		if (parentScan != null) {
			AnnotatedPeakList annotatedPeakList = parentScan
					.getAnnotatedPeakList();
			PeakAnnotationMultiple peakAnnotations = annotatedPeakList
					.getAnnotations(new Peak(scan.getPrecursorMZ(), .0));
			if (peakAnnotations != null) {
				...
				for (Vector<Annotation> structureToAnnotations : peakAnnotations
					...
					for (Annotation annotation : structureToAnnotations) {
						scan.getStructures().addStructure(
	...
	protected void read(InputStream is, boolean merge) throws Exception {
		 ...
		 * Exception("Cannot read from string");
	...
	public void write(OutputStream os) throws Exception {
		 ...
		 * document.appendChild(toXML(document)); XMLUtils.write(fos,document);
		 */
	...
	public static class SAXHandler extends SAXUtils.ObjectTreeHandler {
		...
		protected Object finalizeContent(String namespaceURI, String localName,
			...
			// set glycan document
			GlycanDocument gd = (GlycanDocument) getSubObject(
					GlycanDocument.SAXHandler.getNodeElementName(), false);
			if (gd != null)
				theDocument.currentScan.getStructures().setStructures(
						gd.getStructures());
