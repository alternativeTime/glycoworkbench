...
public class PeakAnnotationSummaryPanel extends SortingTablePanel<AnnotatedPeakList> implements ActionListener, AnnotatedPeakList.AnnotationChangeListener {    
    ...
    public PeakAnnotationSummaryPanel() {
    super();
    }   
    ...
    theActionManager.add("new",GlycoWorkbench.getDefaultThemeManager().getResizableIcon(STOCK_ICON.REFRESH, Plugin.DEFAULT_ICON_SIZE),"Clear all",KeyEvent.VK_N, "",this);
    ...
    theActionManager.add("delete",GlycoWorkbench.getDefaultThemeManager().getResizableIcon("deleteNew", Plugin.DEFAULT_ICON_SIZE),"Delete",-1, "",this);
    ...
    */         
    ...
    public void delete() {
     ...
     if( theDocument.getNoStructures()>0 ) {
        ...
        // clear annotations
        for( Peak p : peaks ) {
        int ind = theDocument.indexOf(p);
        if( theDocument.isAnnotated(p) ) {
            theDocument.clearAnnotationsFor(p);        
            fireRowChanged(theTableSorter.viewIndex(ind));        
        }
        else {
            theDocument.removePeak(p);
