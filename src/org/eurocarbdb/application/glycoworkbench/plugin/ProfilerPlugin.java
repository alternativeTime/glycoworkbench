...
public class ProfilerPlugin implements Plugin, ActionListener,
	...
	public static class DictionariesChangeEvent extends java.util.EventObject {
		...
		public DictionariesChangeEvent(int _action, ProfilerPlugin _source,
				StructureDictionary _changed) {
			super(_source);
	...
	public boolean containsDictionary(StructureDictionary dict) {
		...
		for (StructureDictionary sd : theDictionaries.values()) {
			if (sd.getDictionaryName().equals(dict.getDictionaryName())
					|| sd.getFileName().equals(dict.getFileName()))
				return true;
		}
		return false;
	}
	...
	public void removeDictionary(StructureDictionary dict) {
		...
		theProfilerOptions.USER_DICTIONARIES_FILENAME
				.remove(dict.getFileName());
	...
	public Collection<GlycanAction> getActions() {
		...
		actions.add(getAction("matchDatabase"));
	...
	static private Collection<PeakAnnotation> assertPACollection(Object params)
			throws Exception {
		if (!(params instanceof Collection))
			throw new Exception(
					"Invalid param object: Collection<PeakAnnotation> needed");
	...
	public boolean runAction(boolean ask, String action, Object params)
		...
		if (action.equals("matchStructuresCurrent")) {
			annotate = true;
			merge = true;
			if (matchStructures(ask, assertPeakList(params),
					Collections.singleton(theApplication.getCanvas()
							.getCurrentStructure()))) {
				theManager.show("Annotation", "Details");
				return true;
			}
		...
		if (action.equals("matchStructuresSelected")) {
			annotate = true;
			merge = true;
			if (matchStructures(ask, assertPeakList(params), theApplication
					.getCanvas().getSelectedStructures())) {
				theManager.show("Annotation", "Details");
				return true;
			}
			return false;
		...
		if (action.equals("matchStructuresAll")) {
			annotate = true;
			merge = true;
			if (matchStructures(ask, assertPeakList(params), theWorkspace
		...
		if (action.equals("searchCurrent")) {
			...
			if (search(theApplication.getCanvas().getCurrentStructure(), null,
					false)) {
				theManager.show("Profiler", "Search");
				return true;
			}
		...
		if (action.equals("storeStructuresCurrent")) {
			theApplication.getCanvas().enforceSelection();
			return storeStructures(Collections.singleton(theApplication
					.getCanvas().getCurrentStructure()));
		}
		if (action.equals("storeStructuresSelected")) {
			theApplication.getCanvas().enforceSelection();
			return storeStructures(theApplication.getCanvas()
		...
		// annotations actions
		/*
		 * if( action.equals("storeAnnotations") ) return
		 * storeStructures(getStructures(assertPACollection(params))); else if(
		 * action.equals("editAnnotations") ) { return
		 * editAnnotations(assertPACollection(params)); } else if(
		 * action.equals("removeAnnotations") ) { return
		 * removeAnnotations(assertPACollection(params)); }
		 */
	...
	public boolean setOptions() {
		...
		ProfilerOptionsDialog pdlg = new ProfilerOptionsDialog(theApplication,
				this, theProfilerOptions);
	...
	public boolean matchStructuresDictionaries(boolean ask, PeakList peaks)
			throws Exception {
		if (ask) {
			// show profiler options dialog
		...
		for (int i = 0; i < theProfilerOptions.DICTIONARIES.length; i++)
			generator.add(theDictionaries
					.get(theProfilerOptions.DICTIONARIES[i]));

		// run matching
		return matchStructures(ask, peaks, generator);
	}

	public boolean matchStructures(boolean ask, PeakList peaks,
			StructureGenerator generator) throws Exception {
		...
		// match structures
		theThread = new ProfilerThread(peaks, new Glycan(), generator,
				theProfilerOptions, ann_opt);
	...
	public boolean matchStructures(boolean ask, PeakList peaks,
			Collection<Glycan> structures) throws Exception {
		...
		// match structures
		CollectionStructureGenerator generator = new CollectionStructureGenerator(
				structures);
		theThread = new ProfilerThread(peaks, new Glycan(), generator,
				theProfilerOptions, ann_opt);
	...
	public boolean storeStructures(Collection<Glycan> tostore) throws Exception {
		...
		// open dialog
		StoreStructuresDialog dlg = new StoreStructuresDialog(theApplication,
				this, null);
		dlg.setVisible(true);
	...
	private boolean runAnnotation() {
		...
		progressDialog = new ProgressDialog(theApplication,
				"Matching peaks with structures", null, -1, -1);
		...
		activityMonitor = new javax.swing.Timer(200, new ActionListener() {
			public void actionPerformed(ActionEvent event) {
				// show progress
				progressDialog.setNote(theThread.getProgress()
						+ " structures tested, " + theThread.getMatches()
						+ " matched");

				// check if task is completed or canceled
				if (!theThread.isAlive() || theThread.isInterrupted()
	...
	static private Collection<Glycan> getStructures(
			Collection<PeakAnnotation> pac) {
	...
	public boolean search(Glycan g, StructureDictionary d,
			final boolean search_again) throws MalformedURLException {
		// show dialog
		final SearchDialog dlg = new SearchDialog(theApplication, this,
				search_again, theWorkspace.getGraphicOptions());
	...
	/*
	 * public boolean editAnnotations(Collection<PeakAnnotation> pac) throws
	 * Exception { if( pac.size()==0 ) return false;
	 * 
	 * // check if annotation has valid source PeakAnnotation pa =
	 * pac.iterator().next(); FragmentEntry fe = pa.getFragmentEntry(); if(
	 * fe.isEmpty() || fe.source==null || !(fe.source instanceof StructureType)
	 * ) { JOptionPane.showMessageDialog(theApplication,
	 * "Only the information about structures contained in user databases can be modified."
	 * , "Structure cannot be changed.", JOptionPane.ERROR_MESSAGE); return
	 * false; }
	 * 
	 * // check if structure is in user DB StructureType st =
	 * (StructureType)fe.source; StructureDictionary sd =
	 * theUserDictionaries.get(st.getDatabase()); if( sd==null ) {
	 * JOptionPane.showMessageDialog(theApplication,
	 * "Only the information about structures contained in user databases can be modified."
	 * , "Structure cannot be changed.", JOptionPane.ERROR_MESSAGE); return
	 * false; }
	 * 
	 * // open dialog StoreStructuresDialog dlg = new
	 * StoreStructuresDialog(theApplication,st,this); dlg.setVisible(true); if(
	 * dlg.getReturnStatus().equals("Cancel") ) return false;
	 * 
	 * // edit type st.setType(dlg.getType()); st.setSource(dlg.getSource());
	 * sd.save();
	 * 
	 * JOptionPane.showMessageDialog(theApplication,
	 * "Details of the selected structure have been updated.",
	 * "Details updated.", JOptionPane.INFORMATION_MESSAGE); return true; }
	 * 
	 * public boolean removeAnnotations(Collection<PeakAnnotation> pac) throws
	 * Exception {
	 * 
	 * // ask for confirmation if( JOptionPane.showConfirmDialog(theApplication,
	 * "Are you sure you want to permanently remove the selected structures?"
	 * )!=JOptionPane.YES_OPTION ) return false;
	 * 
	 * // remove structures boolean removed_any = false; boolean nonuserdb_any =
	 * false;
	 * 
	 * for( PeakAnnotation pa : pac) { FragmentEntry fe = pa.getFragmentEntry();
	 * 
	 * // check if annotation has valid source if( fe.isEmpty() ) continue; if(
	 * fe.source==null || !(fe.source instanceof StructureType) ) {
	 * nonuserdb_any = true; continue; }
	 * 
	 * // check if structure is in user DB StructureType st =
	 * (StructureType)fe.source; StructureDictionary sd =
	 * theUserDictionaries.get(st.getDatabase()); if( sd==null ) { nonuserdb_any
	 * = true; continue; }
	 * 
	 * // remove structure from db removed_any = sd.remove(st); }
	 * 
	 * // show results if( removed_any ) { saveUserDictionaries();
	 * 
	 * if( nonuserdb_any ) JOptionPane.showMessageDialog(theApplication,
	 * "Only the selected structures contained in user database(s) have been removed."
	 * , "Structures removed.", JOptionPane.WARNING_MESSAGE); else
	 * JOptionPane.showMessageDialog(theApplication,
	 * "All selected structures have been removed from the user database(s).",
	 * "Structures removed.", JOptionPane.INFORMATION_MESSAGE); } else
	 * JOptionPane.showMessageDialog(theApplication,
	 * "Only structures contained in user databases can be removed.",
	 * "No structures removed.", JOptionPane.ERROR_MESSAGE); return removed_any;
	 * }
	 */
	...
	public void fireDictionariesInit() {
		for (DictionariesChangeListener dcl : dc_listeners)
			dcl.dictionariesChanged(new DictionariesChangeEvent(
					DictionariesChangeEvent.INIT, this, null));
	}

	public void fireDictionaryAdded(StructureDictionary added) {
		for (DictionariesChangeListener dcl : dc_listeners)
			dcl.dictionariesChanged(new DictionariesChangeEvent(
					DictionariesChangeEvent.ADDED, this, added));
	}

	public void fireDictionaryRemoved(StructureDictionary removed) {
		for (DictionariesChangeListener dcl : dc_listeners)
			dcl.dictionariesChanged(new DictionariesChangeEvent(
					DictionariesChangeEvent.REMOVED, this, removed));
	}

	public void fireDictionaryChanged(StructureDictionary changed) {
		for (DictionariesChangeListener dcl : dc_listeners)
			dcl.dictionariesChanged(new DictionariesChangeEvent(
					DictionariesChangeEvent.CHANGED, this, changed));
	}
