...
public class SearchDetailsPanel extends SortingTablePanel<AnnotatedPeakList> implements ActionListener {    
    ...
    */         
    ...
    for( PeakAnnotation pa : to_remove ) {
        ...
        int ind = theDocument.removePeakAnnotation(current_ind,pa);
    ...
    PeakAnnotationCollection pac = theDocument.getPeakAnnotationCollection(current_ind);
    if( focusRow!=-1 ) 
        ret.add(pac.elementAt(theTableSorter.modelIndex(focusRow)));
    for( int row : sel_inds ) {
        if( row!=focusRow ) 
        ret.add(pac.elementAt(theTableSorter.modelIndex(row)));    
    }
    ...
    int[] sel_ind = theTable.getSelectedRows();
    if( sel_ind.length>0 ) {
        Glycan structure = theDocument.getStructure(current_ind);
        PeakAnnotationCollection pac = theDocument.getPeakAnnotationCollection(current_ind);
        
        Vector<Peak> p_toadd = new Vector<Peak>();
        PeakAnnotationCollection pac_toadd = new PeakAnnotationCollection();
        for( int i=0; i<sel_ind.length; i++ ) {
        int r = theTableSorter.modelIndex(sel_ind[i]);

        pac_toadd.addPeakAnnotation(pac.getPeakAnnotation(r));
        p_toadd.add(pac.getPeak(r));
        }

        theWorkspace.getAnnotatedPeakList().addPeakAnnotations(structure,pac_toadd,true);
