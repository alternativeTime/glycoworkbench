...
public class StructureDictionary extends BaseDocument {
	...
	public StructureDictionary(String _filename, boolean _on_file_system,
			StructureScorer scorer) throws Exception {
		super(false);
		load(_filename, _on_file_system, scorer);
	}

	public StructureDictionary(InputStream is, StructureScorer scorer)
			throws Exception {
		super(false);
		load(is, scorer);
	}
	...
	public Collection<javax.swing.filechooser.FileFilter> getFileFormats() {
		...
		filters.add(new ExtensionFileFilter(new String[] { "gwd" },
				"GlycoWorkbench dictionary file"));
		return filters;
	}

	public javax.swing.filechooser.FileFilter getAllFileFormats() {
		return new ExtensionFileFilter(new String[] { "gwd" },
				"GlycoWorkbench dictionary file");
	}
	...
	public boolean addAll(Collection<Glycan> gs, String type, String source) {
		boolean added = false;
		for (Glycan g : gs) {
			if (addPVT(new StructureType(this.dictionary_name, type, source,
					g.toString())))
				added = true;
		}
	...
	public FragmentCollection generateStructures(ProfilerOptions opt)
			throws Exception {
	...
	public void generateStructures(FragmentCollection fc, ProfilerOptions opt)
			throws Exception {
	...
	public boolean load(String _filename, boolean _on_file_system,
			StructureScorer scorer) {
		try {
			load(FileUtils.open(this.getClass(), _filename, !_on_file_system),
					scorer);
	...
	protected void read(InputStream is, boolean merge) throws Exception {
		/*
		 * Document document = XMLUtils.read(is); if( document==null ) throw new
		 * Exception("Cannot read from stream");
		 * fromXML(XMLUtils.assertChild(document
		 * ,"StructureDictionary"),merge,null);
		 */
		SAXUtils.read(is, new SAXHandler(this, merge));
	}

	public static StructureDictionary fromXML(Node d_node,
			StructureScorer scorer) throws Exception {
	...
	public void fromXML(Node d_node, boolean merge, StructureScorer scorer)
			throws Exception {
		if (!merge)
			dictionary.clear();
		...
		// read types
		Vector<Node> st_nodes = XMLUtils.findAllChildren(d_node,
				"StructureType");
	...
	public static class SAXHandler extends SAXUtils.ObjectTreeHandler {
		...
		protected Object finalizeContent(String namespaceURI, String localName,
			...
			for (Object o : getSubObjects(StructureType.SAXHandler
					.getNodeElementName())) {
